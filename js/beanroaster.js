...
var onclickListener = function (evt) {
    ...
    if (strRGBActive != numLastRgbVal) {
        ...
        document.getElementById('dvActiveColour').dataset.colour =
            strRGBActive + ',' + decRGBRed + ',' + decRGBGreen + ',' + decRGBBlue;
...
function getClosestMatch(arrColourResults, intDecRed, intDecGreen, intDecBlue) {
    ...
    for (var i = 0; i < arrColourResults.length; i++) {
        var arrAbsDiffVals = [
            Math.abs(intDecRed - arrColourResults[i][2]),
            Math.abs(intDecGreen - arrColourResults[i][3]),
            Math.abs(intDecBlue - arrColourResults[i][4])
        ];
        arrAbsDiffVals.sort(function (a, b) {
            return a - b;
        });
        ...
        if (intCurrMaxDeviation < intMaxDeviation) {
            ...
            if (intCurrMaxDeviation == intMaxDeviation && intCurrMidDeviation < intMidDeviation) {
                ...
                if (
                    intCurrMaxDeviation == intMaxDeviation &&
                    intCurrMidDeviation == intMidDeviation &&
                    intCurrMinDeviation < intMinDeviation
                ) {
                    arrLowPosList = [i];
                    intMinDeviation = intCurrMinDeviation;
                } else {
...
function loadCanvas() {
    ...
    var onmoveListener = function (evt) {
        ...
        if (istat == false) {
            ...
            drawPix(
                ...
                ctxPix,
                img,
                Math.round((mousePos.x + cnvWidth - 470) * (imgWidth / cnvWidth)),
    ...
    var onMiniclickListener = function (evt) {
        ...
        if (strRGBActive != numLastRgbVal) {
            ...
            document.getElementById('dvActiveColour').style.backgroundColor = strRGBActive;
            document.getElementById('dvActiveColour').dataset.colour =
                strRGBActive + ',' + decZoomRGBRed + ',' + decZoomRGBGreen + ',' + decZoomRGBBlue;
